...
local function BuildRequest(URL, FormData, ExtraHeaders, Host, Method)
	local Request = (Method or "POST ") .. URL .. " HTTP/1.1\n";
...
local function Login(File)
	local Config = require "lapis.config".get();
	local Username, Password = Config.robloxun, Config.robloxpw;
	...
	local SessionFile, Error = io.open("security.sec", "w");
...
return {BuildRequest, HTTPRequestSSL, HTTPRequest, StripHeaders, Login, DataRequest, RunPostBack, FindPostState, HTTPGet};
